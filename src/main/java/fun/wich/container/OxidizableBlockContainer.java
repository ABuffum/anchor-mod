...
public class OxidizableBlockContainer implements IContainer {
	...
	public OxidizableBlockContainer(OxidizableBlockSupplier oxidizable, Function<AbstractBlock.Settings, Block> waxed, OxidationScale.BlockSettingsSupplier settings, Item.Settings itemSettings) {
		unaffected = new BlockContainer(oxidizable.get(OxidizationLevel.UNAFFECTED, settings.get(OxidizationLevel.UNAFFECTED)), itemSettings);
		exposed = new BlockContainer(oxidizable.get(OxidizationLevel.EXPOSED, settings.get(OxidizationLevel.EXPOSED)), itemSettings);
		weathered = new BlockContainer(oxidizable.get(OxidizationLevel.WEATHERED, settings.get(OxidizationLevel.WEATHERED)), itemSettings);
		oxidized = new BlockContainer(oxidizable.get(OxidizationLevel.OXIDIZED, settings.get(OxidizationLevel.OXIDIZED)), itemSettings);
		waxed_unaffected = new WaxedBlockContainer(unaffected, waxed.apply(settings.get(OxidizationLevel.UNAFFECTED)), itemSettings);
		waxed_exposed = new WaxedBlockContainer(exposed, waxed.apply(settings.get(OxidizationLevel.EXPOSED)), itemSettings);
		waxed_weathered = new WaxedBlockContainer(weathered, waxed.apply(settings.get(OxidizationLevel.WEATHERED)), itemSettings);
		waxed_oxidized = new WaxedBlockContainer(oxidized, waxed.apply(settings.get(OxidizationLevel.OXIDIZED)), itemSettings);
	}
	...
	public OxidizableBlockContainer(OxidizableBlockSupplier oxidizable, Function<AbstractBlock.Settings, Block> waxed, AbstractBlock.Settings settings, Item.Settings itemSettings) {
		unaffected = new BlockContainer(oxidizable.get(OxidizationLevel.UNAFFECTED, settings), itemSettings);
		exposed = new BlockContainer(oxidizable.get(OxidizationLevel.EXPOSED, settings), itemSettings);
		weathered = new BlockContainer(oxidizable.get(OxidizationLevel.WEATHERED, settings), itemSettings);
		oxidized = new BlockContainer(oxidizable.get(OxidizationLevel.OXIDIZED, settings), itemSettings);
