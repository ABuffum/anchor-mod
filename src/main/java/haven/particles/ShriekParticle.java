...
public class ShriekParticle extends SpriteBillboardParticle {
	...
	public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
		...
		this.colorAlpha = 1.0f - MathHelper.clamp(((float)this.age + tickDelta) / (float)this.maxAge, 0.0f, 1.0f);
	...
	private void method_42584(VertexConsumer vertexConsumer, Vec3f vec3f, float f, float g, int i) {
		vertexConsumer.vertex(vec3f.getX(), vec3f.getY(), vec3f.getZ()).texture(f, g).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(i).next();
	}
