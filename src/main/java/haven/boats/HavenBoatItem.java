...
public class HavenBoatItem extends Item {
	private static final Predicate<Entity> RIDERS;
	private final HavenBoatType type;

	public HavenBoatItem(HavenBoatType type, Item.Settings settings) {
		super(settings);
		this.type = type;
	}
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (hitResult.getType() == HitResult.Type.MISS) {
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				HavenBoatEntity boatEntity = new HavenBoatEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z);
