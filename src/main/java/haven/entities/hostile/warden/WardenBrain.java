...
public class WardenBrain {
	...
	private static final List<MemoryModuleType<?>> MEMORY_MODULES = List.of(MemoryModuleType.MOBS, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_NEMESIS, MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.ATTACK_TARGET, MemoryModuleType.ATTACK_COOLING_DOWN, MemoryModuleType.NEAREST_ATTACKABLE, MemoryModules.ROAR_TARGET, MemoryModules.DISTURBANCE_LOCATION, MemoryModules.RECENT_PROJECTILE, MemoryModules.IS_SNIFFING, MemoryModules.IS_EMERGING, MemoryModules.ROAR_SOUND_DELAY, MemoryModules.DIG_COOLDOWN, MemoryModules.ROAR_SOUND_COOLDOWN, MemoryModules.SNIFF_COOLDOWN, MemoryModules.TOUCH_COOLDOWN, MemoryModules.VIBRATION_COOLDOWN, MemoryModules.SONIC_BOOM_COOLDOWN, MemoryModules.SONIC_BOOM_SOUND_COOLDOWN, MemoryModules.SONIC_BOOM_SOUND_DELAY);
	private static final Task<WardenEntity> RESET_DIG_COOLDOWN_TASK = new Task<WardenEntity>(ImmutableMap.of(MemoryModules.DIG_COOLDOWN, MemoryModuleState.REGISTERED)){
	...
	public static void updateActivities(WardenEntity warden) {
		warden.getBrain().resetPossibleActivities(ImmutableList.of(Activities.EMERGE, Activities.DIG, Activities.ROAR, Activity.FIGHT, Activities.INVESTIGATE, Activities.SNIFF, Activity.IDLE));
	}
	...
	private static void addEmergeActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activities.EMERGE, 5, ImmutableList.of(new EmergeTask(EMERGE_DURATION)), MemoryModules.IS_EMERGING);
	}

	private static void addDigActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activities.DIG,
				ImmutableList.of(Pair.of(0, new DismountVehicleTask()), Pair.of(1, new DigTask(DIG_DURATION))),
				ImmutableSet.of(
						Pair.of(MemoryModules.ROAR_TARGET, MemoryModuleState.VALUE_ABSENT),
						Pair.of(MemoryModules.DIG_COOLDOWN, MemoryModuleState.VALUE_ABSENT)));
	}
	...
	private static void addIdleActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activity.IDLE, 10,
				ImmutableList.of(
						...
						new RandomTask<WardenEntity>(
								ImmutableMap.of(MemoryModules.IS_SNIFFING, MemoryModuleState.VALUE_ABSENT),
	...
	private static void addInvestigateActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activities.INVESTIGATE, 5,
				ImmutableList.of(
						new FindRoarTargetTask(WardenEntity::getPrimeSuspect),
						new WardenGoToCelebrateTask(MemoryModules.DISTURBANCE_LOCATION, 2, 0.7f)),
				MemoryModules.DISTURBANCE_LOCATION);
	}

	private static void addSniffActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activities.SNIFF, 5,
				ImmutableList.of(
						new FindRoarTargetTask(WardenEntity::getPrimeSuspect),
						new SniffTask(SNIFF_DURATION)), MemoryModules.IS_SNIFFING);
	}

	private static void addRoarActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(Activities.ROAR, 10, ImmutableList.of(new RoarTask()), MemoryModules.ROAR_TARGET);
	}
	...
	private static void addFightActivities(WardenEntity warden, Brain<WardenEntity> brain) {
		brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(RESET_DIG_COOLDOWN_TASK,
				new ForgetAttackTargetTask<WardenEntity>(
						entity -> !warden.getAngriness().isAngry() || !warden.isValidTarget((Entity)entity),
						entity -> removeDeadSuspect(warden, entity)
						//TODO: Warden should not forget just because the target is unreachable
						//, false
				),
				new FollowMobTask(
						entity -> WardenBrain.isTargeting(warden, entity),
	...
	public static void removeDeadSuspect(WardenEntity warden, LivingEntity suspect) {
	...
	public static void resetDigCooldown(LivingEntity warden) {
		if (warden.getBrain().hasMemoryModule(MemoryModules.DIG_COOLDOWN)) {
			warden.getBrain().remember(MemoryModules.DIG_COOLDOWN, Unit.INSTANCE, 1200L);
		}
	}
	...
	public static void lookAtDisturbance(WardenEntity warden, BlockPos pos) {
		...
		WardenBrain.resetDigCooldown(warden);
		warden.getBrain().remember(MemoryModules.SNIFF_COOLDOWN, Unit.INSTANCE, 100L);
		warden.getBrain().remember(MemoryModuleType.LOOK_TARGET, new BlockPosLookTarget(pos), 100L);
		warden.getBrain().remember(MemoryModules.DISTURBANCE_LOCATION, pos, 100L);
