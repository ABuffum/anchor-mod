...
public class WarpedMooshroomEntity extends CowEntity implements Shearable {
	public WarpedMooshroomEntity(EntityType<? extends WarpedMooshroomEntity> entityType, World world) {
		super(entityType, world);
	}

	public float getPathfindingFavor(BlockPos pos, WorldView world) {
		return world.getBlockState(pos.down()).isOf(Blocks.WARPED_NYLIUM) ? 10.0F : world.getBrightness(pos) - 0.5F;
	}

	public static boolean canSpawn(EntityType<WarpedMooshroomEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
		return world.getBlockState(pos.down()).isOf(Blocks.WARPED_NYLIUM) && world.getBaseLightLevel(pos, 0) > 8;
	}
	...
	public void sheared(SoundCategory shearedSoundCategory) {
		...
		if (!this.world.isClient()) {
			...
			for(int i = 0; i < 5; ++i) {
				this.world.spawnEntity(new ItemEntity(this.world, this.getX(), this.getBodyY(1.0D), this.getZ(), new ItemStack(Items.WARPED_FUNGUS)));
			}
		}
	...
	public WarpedMooshroomEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		WarpedMooshroomEntity warpedMooshroomEntity = (WarpedMooshroomEntity)HavenMod.WARPED_MOOSHROOM_ENTITY.create(serverWorld);
		return warpedMooshroomEntity;
	}
}