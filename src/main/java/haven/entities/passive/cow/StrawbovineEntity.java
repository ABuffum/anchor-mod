...
public class StrawbovineEntity extends CowEntity implements FlavoredCowEntity {

	@Override
	public CakeContainer.Flavor getFlavor() { return CakeContainer.Flavor.STRAWBERRY; }

	public StrawbovineEntity(EntityType<? extends StrawbovineEntity> entityType, World world) {
	...
	protected void initGoals() {
		...
		this.goalSelector.add(3, new TemptGoal(this, 1.25D, Ingredient.ofItems(Items.WHEAT, HavenMod.STRAWBERRY), false));
	}
	...
	public static boolean canSpawn(EntityType<StrawbovineEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack != null && !itemStack.isEmpty() && !this.isBaby()) {
			...
			if (item instanceof BucketProvided bp) outItem = bp.getBucketProvider().getStrawberryMilkBucket();
			else if (item == Items.BOWL) outItem = HavenMod.STRAWBERRY_MILK_BOWL;
			if (outItem != null) {
	...
	public StrawbovineEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		return HavenMod.STRAWBOVINE_ENTITY.create(serverWorld);
	}
