...
public class WhiteMooblossomEntity extends CowEntity implements Shearable {
	public WhiteMooblossomEntity(EntityType<? extends WhiteMooblossomEntity> entityType, World world) {
	...
	public static boolean canSpawn(EntityType<WhiteMooblossomEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
	...
	public void sheared(SoundCategory shearedSoundCategory) {
		...
		if (!this.world.isClient()) {
			...
			for(int i = 0; i < 5; ++i) {
				this.world.spawnEntity(new ItemEntity(this.world, this.getX(), this.getBodyY(1.0D), this.getZ(), new ItemStack(Items.WHITE_TULIP)));
			}
		}
	...
	public WhiteMooblossomEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		return HavenMod.WHITE_MOOBLOSSOM_ENTITY.create(serverWorld);
	}
