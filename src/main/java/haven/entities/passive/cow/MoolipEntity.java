...
public class MoolipEntity extends CowEntity implements Shearable {
	public MoolipEntity(EntityType<? extends MoolipEntity> entityType, World world) {
	...
	public static boolean canSpawn(EntityType<MoolipEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
	...
	public void sheared(SoundCategory shearedSoundCategory) {
		...
		if (!this.world.isClient()) {
			...
			for(int i = 0; i < 5; ++i) {
				this.world.spawnEntity(new ItemEntity(this.world, this.getX(), this.getBodyY(1.0D), this.getZ(), new ItemStack(ModBase.PINK_DAISY.getItem())));
			}
		}
	...
	public MoolipEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		MoolipEntity moolipEntity = (MoolipEntity) ModBase.MOOLIP_ENTITY.create(serverWorld);
		return moolipEntity;
	}
}