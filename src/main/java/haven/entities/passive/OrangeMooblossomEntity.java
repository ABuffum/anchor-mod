...
public class OrangeMooblossomEntity extends CowEntity implements Shearable {
	public OrangeMooblossomEntity(EntityType<? extends OrangeMooblossomEntity> entityType, World world) {
	...
	public static boolean canSpawn(EntityType<OrangeMooblossomEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
	...
	public void sheared(SoundCategory shearedSoundCategory) {
		...
		if (!this.world.isClient()) {
			...
			for(int i = 0; i < 5; ++i) {
				this.world.spawnEntity(new ItemEntity(this.world, this.getX(), this.getBodyY(1.0D), this.getZ(), new ItemStack(Items.ORANGE_TULIP)));
			}
		}
	...
	public OrangeMooblossomEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		OrangeMooblossomEntity mooblossomEntity = (OrangeMooblossomEntity)HavenMod.ORANGE_MOOBLOSSOM_ENTITY.create(serverWorld);
