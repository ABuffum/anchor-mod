...
import haven.entities.OrangeMooblossomEntity;
...
public class OrangeMooblossomFlowerFeatureRenderer<T extends OrangeMooblossomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public OrangeMooblossomFlowerFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T mooblossomEntity, float f, float g, float h, float j, float k, float l) {
		if (!mooblossomEntity.isBaby()) {
			...
			if (!mooblossomEntity.isInvisible() || bl) {
				...
				BlockState blockState = HavenMod.ORANGE_MOOBLOSSOM_TULIP.getDefaultState();
				...
				this.renderOrangeMublossom(matrixStack, vertexConsumerProvider, i, bl, blockRenderManager, blockState, m, bakedModel);
				matrixStack.pop();
				matrixStack.push();
				matrixStack.translate(0.20000000298023224D, -0.3499999940395355D, 0.5D);
	...
	private void renderOrangeMublossom(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, boolean renderAsModel, BlockRenderManager blockRenderManager, BlockState mublossomState, int overlay, BakedModel mublossomModel) {
		if (renderAsModel) {
