...
public class PinkMooblossomFlowerFeatureRenderer<T extends PinkMooblossomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public PinkMooblossomFlowerFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T mooblossomEntity, float f, float g, float h, float j, float k, float l) {
		if (!mooblossomEntity.isBaby()) {
			...
			if (!mooblossomEntity.isInvisible() || bl) {
				...
				BlockState blockState = HavenMod.PINK_MOOBLOSSOM_TULIP.getDefaultState();
				...
				this.renderPinkMublossom(matrixStack, vertexConsumerProvider, i, bl, blockRenderManager, blockState, m, bakedModel);
				matrixStack.pop();
				matrixStack.push();
				matrixStack.translate(0.20000000298023224D, -0.3499999940395355D, 0.5D);
	...
	private void renderPinkMublossom(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, boolean renderAsModel, BlockRenderManager blockRenderManager, BlockState mublossomState, int overlay, BakedModel mublossomModel) {
		if (renderAsModel) {
