...
public class CrimsonMooshroomMushroomFeatureRenderer<T extends CrimsonMooshroomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public CrimsonMooshroomMushroomFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T crimsonMooshroomEntity, float f, float g, float h, float j, float k, float l) {
		if (!crimsonMooshroomEntity.isBaby()) {
			...
			if (!crimsonMooshroomEntity.isInvisible() || bl) {
				...
				BlockState blockState = Blocks.CRIMSON_FUNGUS.getDefaultState();
