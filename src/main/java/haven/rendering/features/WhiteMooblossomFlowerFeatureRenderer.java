...
public class WhiteMooblossomFlowerFeatureRenderer<T extends WhiteMooblossomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public WhiteMooblossomFlowerFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T mooblossomEntity, float f, float g, float h, float j, float k, float l) {
		if (!mooblossomEntity.isBaby()) {
			...
			if (!mooblossomEntity.isInvisible() || bl) {
				...
				BlockState blockState = HavenMod.WHITE_MOOBLOSSOM_TULIP.getDefaultState();
				...
				this.renderWhiteMublossom(matrixStack, vertexConsumerProvider, i, bl, blockRenderManager, blockState, m, bakedModel);
				matrixStack.pop();
				matrixStack.push();
				matrixStack.translate(0.20000000298023224D, -0.3499999940395355D, 0.5D);
	...
	private void renderWhiteMublossom(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, boolean renderAsModel, BlockRenderManager blockRenderManager, BlockState mublossomState, int overlay, BakedModel mublossomModel) {
		if (renderAsModel) {
