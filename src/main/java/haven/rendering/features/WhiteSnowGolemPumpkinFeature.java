...
public class WhiteSnowGolemPumpkinFeature extends FeatureRenderer<WhiteSnowGolemEntity, SnowGolemEntityModel<WhiteSnowGolemEntity>> {
	public WhiteSnowGolemPumpkinFeature(FeatureRendererContext<WhiteSnowGolemEntity, SnowGolemEntityModel<WhiteSnowGolemEntity>> featureRendererContext) {
		super(featureRendererContext);
	}

	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, WhiteSnowGolemEntity snowGolemEntity, float f, float g, float h, float j, float k, float l) {
		if (snowGolemEntity.hasPumpkin()) {
			...
			if (!snowGolemEntity.isInvisible() || bl) {
				...
				ItemStack itemStack = new ItemStack(ModBase.WHITE_PUMPKIN.getCarved().getBlock());
				if (bl) {
					BlockState blockState = ModBase.WHITE_PUMPKIN.getCarved().getBlock().getDefaultState();
