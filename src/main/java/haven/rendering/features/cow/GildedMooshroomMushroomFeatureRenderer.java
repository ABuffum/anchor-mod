...
public class GildedMooshroomMushroomFeatureRenderer<T extends GildedMooshroomEntity> extends FeatureRenderer<T, CowEntityModel<T>> {
	public GildedMooshroomMushroomFeatureRenderer(FeatureRendererContext<T, CowEntityModel<T>> featureRendererContext) {
		super(featureRendererContext);
	}

	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T gildedMooshroomEntity, float f, float g, float h, float j, float k, float l) {
		if (!gildedMooshroomEntity.isBaby()) {
			MinecraftClient minecraftClient = MinecraftClient.getInstance();
			boolean bl = minecraftClient.hasOutline(gildedMooshroomEntity) && gildedMooshroomEntity.isInvisible();
			if (!gildedMooshroomEntity.isInvisible() || bl) {
				BlockRenderManager blockRenderManager = minecraftClient.getBlockRenderManager();
				BlockState blockState = ModBase.GILDED_MATERIAL.getFungus().getBlock().getDefaultState();
				int m = LivingEntityRenderer.getOverlay(gildedMooshroomEntity, 0.0F);
