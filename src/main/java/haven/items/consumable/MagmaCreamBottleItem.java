...
public class MagmaCreamBottleItem extends Item {
	public MagmaCreamBottleItem(Settings settings) {
		super(settings);
	}
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		...
		if (!world.isClient) {
			...
			if (bloodType == BloodType.MAGMA) user.heal(2);
			else if (bloodType == BloodType.LAVA) user.heal(1);
			else if (bloodType == BloodType.NETHER) { }
			else if (bloodType == BloodType.NETHER_ROYALTY) { }
			else {
				if (bloodType.IsFireVulnerable()){
					user.damage(HavenDamageSource.DRANK_MAGMA_CREAM, 4);
