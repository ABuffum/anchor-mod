...
public class SludgeBottleItem extends Item {
	public SludgeBottleItem(Settings settings) {
		super(settings);
	}
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		...
		if (!world.isClient) {
			...
			if (bloodType == BloodType.SLUDGE) user.heal(2);
			else if (bloodType != BloodType.SLIME) {
				user.damage(HavenDamageSource.DRANK_SLUDGE, 8);
				user.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 600, 2));
				user.addStatusEffect(new StatusEffectInstance(StatusEffects.WITHER, 600, 2));
				user.addStatusEffect(new StatusEffectInstance(StatusEffects.HUNGER, 600, 2));
				//TODO: Sticky
			}
		}
