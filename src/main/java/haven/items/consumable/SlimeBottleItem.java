...
public class SlimeBottleItem extends Item {
	public SlimeBottleItem(Settings settings) {
		super(settings);
	}
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		...
		if (!world.isClient) {
			...
			if (bloodType == BloodType.SLIME) user.heal(2);
			else if (bloodType == BloodType.SLUDGE) user.heal(1);
			else {
				user.damage(HavenDamageSource.DRANK_SLIME, 4);
				//TODO: Sticky
			}
		}
