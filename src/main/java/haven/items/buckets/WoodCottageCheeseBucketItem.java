...
public class WoodCottageCheeseBucketItem extends Item {
	private boolean canDrink;

	private final Block block;
	public WoodCottageCheeseBucketItem(Block block, Settings settings) {
		super(settings);
		this.block = block;
	}
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (actionResult.isAccepted() && playerEntity != null && !playerEntity.isCreative()) {
			...
			playerEntity.setStackInHand(hand, HavenMod.WOOD_BUCKET.getDefaultStack());
		}
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (hit != null && hit.getType() == HitResult.Type.BLOCK && !user.isSneaking()) {
			...
			if (world.getBlockState(hit.getBlockPos()).getBlock() instanceof CauldronBlock) {
				user.setStackInHand(hand, new ItemStack(HavenMod.WOOD_BUCKET));
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		...
		if (playerEntity == null || !playerEntity.getAbilities().creativeMode) {
			if (stack.isEmpty()) {
				return new ItemStack(HavenMod.WOOD_BUCKET);
			}
			if (playerEntity != null) {
				playerEntity.getInventory().insertStack(new ItemStack(HavenMod.WOOD_BUCKET));
			}
		}
	...
	public ItemStack getDefaultStack() {
		return new ItemStack(HavenMod.WOOD_COTTAGE_CHEESE_BUCKET);
	}
