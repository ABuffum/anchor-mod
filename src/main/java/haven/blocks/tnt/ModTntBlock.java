...
public abstract class ModTntBlock extends Block {
	...
	protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
		builder.add(Properties.UNSTABLE);
	}

	public interface ITntEntityProvider {
		public ModTntEntity Make(World world, double x, double y, double z, LivingEntity igniter);
	}

	public static ItemDispenserBehavior DispenserBehavior(ITntEntityProvider provider) {
		return new ItemDispenserBehavior() {
			protected ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
				World world = pointer.getWorld();
				BlockPos blockPos = pointer.getPos().offset(pointer.getBlockState().get(DispenserBlock.FACING));
				ModTntEntity tntEntity = provider.Make(world, (double) blockPos.getX() + 0.5D, blockPos.getY(), (double) blockPos.getZ() + 0.5D, null);
				world.spawnEntity(tntEntity);
				if (tntEntity.makeSound()) {
