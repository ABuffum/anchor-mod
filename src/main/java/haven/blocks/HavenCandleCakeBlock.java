...
public class HavenCandleCakeBlock extends AbstractCandleBlock {
	...
	public HavenCandleCakeBlock(Flavor flavor) {
		this(flavor, HavenCakeBlock.SETTINGS);
	}
	public HavenCandleCakeBlock(Flavor flavor, AbstractBlock.Settings settings) {
		super(settings);
		this.flavor = flavor;
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		...
		if (!itemStack.isOf(Items.FLINT_AND_STEEL) && !itemStack.isOf(Items.FIRE_CHARGE)) {
			if (isHittingCandle(hit) && player.getStackInHand(hand).isEmpty() && (Boolean)state.get(LIT)) {
				...
				Block output;
				if (flavor == Flavor.CHOCOLATE) output = HavenMod.CHOCOLATE_CAKE.BLOCK;
