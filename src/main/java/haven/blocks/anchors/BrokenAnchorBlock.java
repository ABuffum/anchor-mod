...
public class BrokenAnchorBlock extends BlockWithEntity {
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return checkType(type, ModBase.BROKEN_ANCHOR_BLOCK_ENTITY, BrokenAnchorBlockEntity::tick);
	}
	...
	public void afterBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity, ItemStack stack) {
		...
		if (ModBase.ANCHOR_MAP.containsKey(owner)) {
			ItemStack otherStack = new ItemStack(ModBase.BROKEN_ANCHOR_CORES.get(owner), 1);
