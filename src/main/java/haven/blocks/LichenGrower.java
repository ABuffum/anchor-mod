...
public class LichenGrower {
	...
	public boolean canGrow(BlockState state, BlockView world, BlockPos pos, Direction direction) {
		return Stream.of(Direction.values()).anyMatch(direction2 -> this.getGrowPos(state, world, pos, direction, (Direction)direction2, this.growChecker::canGrow).isPresent());
	}

	public Optional<GrowPos> grow(BlockState state, WorldAccess world, BlockPos pos, Random random) {
		return CollectionUtil.copyShuffled(Direction.values(), random).stream().filter(direction -> this.growChecker.canGrow(state, (Direction)direction)).map(direction -> this.grow(state, world, pos, (Direction)direction, random, false)).filter(Optional::isPresent).findFirst().orElse(Optional.empty());
	}

	public long grow(BlockState state, WorldAccess world, BlockPos pos, boolean markForPostProcessing) {
		return Stream.of(Direction.values()).filter(direction -> this.growChecker.canGrow(state, (Direction)direction)).map(direction -> this.grow(state, world, pos, (Direction)direction, markForPostProcessing)).reduce(0L, Long::sum);
	}
	...
	public Optional<GrowPos> grow(BlockState state, WorldAccess world, BlockPos pos, Direction direction, Random random, boolean markForPostProcessing) {
		return CollectionUtil.copyShuffled(Direction.values(), random).stream().map(direction2 -> this.grow(state, world, pos, direction, (Direction)direction2, markForPostProcessing)).filter(Optional::isPresent).findFirst().orElse(Optional.empty());
	}

	private long grow(BlockState state, WorldAccess world, BlockPos pos, Direction direction, boolean markForPostProcessing) {
		return Stream.of(Direction.values()).map(direction2 -> this.grow(state, world, pos, direction, (Direction)direction2, markForPostProcessing)).filter(Optional::isPresent).count();
	}
