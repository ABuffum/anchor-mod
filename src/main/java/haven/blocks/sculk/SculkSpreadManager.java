...
public class SculkSpreadManager {
	...
	private void addCursor(Cursor cursor) {
		if (this.cursors.size() >= 32) return;
		this.cursors.add(cursor);
	}

	public void tick(World world, BlockPos pos2, Random random, boolean shouldConvertToBlock) {
		BlockPos blockPos;
		if (this.cursors.isEmpty()) return;
		...
		for (Cursor cursor : this.cursors) {
			...
			if (cursor.charge <= 0) {
				world.syncWorldEvent(HavenWorldEvents.SCULK_CHARGE, cursor.getPos(), 0);
				continue;
			}
		...
		for (Object2IntMap.Entry entry : object2IntMap.object2IntEntrySet()) {
			...
			world.syncWorldEvent(HavenWorldEvents.SCULK_CHARGE, blockPos, k);
		}
		this.cursors = list;
	}
	...
	public static class Cursor {
		...
		public static final Codec<Cursor> CODEC = RecordCodecBuilder.create(instance ->
				instance.group(
						...
						.apply(instance, (pos, charge, decay, update, faces) ->
								new Cursor((BlockPos)pos, (int)charge, (int)decay, (int)update, (Optional<Set<Direction>>)faces)));
		...
		private boolean canSpread(World world, BlockPos pos, boolean worldGen) {
			if (this.charge <= 0) return false;
			if (worldGen) return true;
			if (world instanceof ServerWorld serverWorld) {
				return serverWorld.getChunkManager().isTickingFutureReady(ChunkPos.toLong(pos));
			}
			return false;
		}

		public void spread(World world, BlockPos pos, Random random, SculkSpreadManager spreadManager, boolean shouldConvertToBlock) {
			if (!this.canSpread(world, pos, spreadManager.worldGen)) return;
			if (this.update > 0) {
				--this.update;
				return;
			}
			...
			if (blockState.getBlock() instanceof SculkSpreadable) this.faces = MultifaceGrowthBlock.collectDirections(blockState);
		...
		@Nullable
		private static BlockPos getSpreadPos(World world, BlockPos pos, Random random) {
		...
		private static boolean canSpread(World world, BlockPos sourcePos, BlockPos targetPos) {
			...
			if (blockPos.getX() == 0) return Cursor.canSpread(world, sourcePos, direction2) || Cursor.canSpread(world, sourcePos, direction3);
			if (blockPos.getY() == 0) return Cursor.canSpread(world, sourcePos, direction) || Cursor.canSpread(world, sourcePos, direction3);
			return Cursor.canSpread(world, sourcePos, direction) || Cursor.canSpread(world, sourcePos, direction2);
		}

		private static boolean canSpread(World world, BlockPos pos, Direction direction) {
